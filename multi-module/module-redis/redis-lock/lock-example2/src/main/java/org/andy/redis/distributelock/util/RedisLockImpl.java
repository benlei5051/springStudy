        ...
        while (true) { //循环获取锁
            ...
            if (redisTemplate.execute(new RedisCallback<Boolean>() {
                ...
                public Boolean doInRedis(RedisConnection redisConnection) throws DataAccessException {
                    ...
                    byte[] value = serializer.serialize(lockTimeout.toString());
                ...
                //锁已经失效
                if (currtLockTimeoutStr != null && currtLockTimeoutStr < System.currentTimeMillis()) {
                    //判断是否为空，不为空时，说明已经失效，如果被其他线程设置了值，则第二个条件判断无法执行
                    //获取上一个锁到期时间，并设置现在的锁到期时间
                    Long old_lock_timeout_Str = Long.valueOf(redisTemplate.opsForValue().getAndSet(lockKey, lockTimeout.toString()));
                    if (old_lock_timeout_Str != null && old_lock_timeout_Str.equals(currtLockTimeoutStr)) {
                        ...
                        logger.info(threadName + "加锁成功 ++++ 22222");
                        //设置超时间，释放内存
                        redisTemplate.expire(lockKey, LOCK_TIMEOUT, TimeUnit.MILLISECONDS);
...
//        TimeUnit.MILLISECONDS.sleep(100);
    ...
    public synchronized void unlock(String lockKey, long lockValue, String threadName) {
        logger.info(threadName + "执行解锁==========");//正常直接删除 如果异常关闭判断加锁会判断过期时间
        //获取redis中设置的时间
        String result = redisTemplate.opsForValue().get(lockKey);
